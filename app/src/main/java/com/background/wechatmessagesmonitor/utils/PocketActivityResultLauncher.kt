...
) = launch(Intent(context, T::class.java).putExtras(bundleOf(*values)))

inline fun <reified T> PocketActivityResultLauncher<Intent, *>.launch(
    context: Context,
    options: ActivityOptionsCompat?,
    vararg values: Pair<String, Any?>
) = launch(Intent(context, T::class.java).putExtras(bundleOf(*values)), options)

inline fun <reified T, R> PocketActivityResultLauncher<Intent, R>.launch(
    context: Context,
    vararg values: Pair<String, Any?>,
    callback: ActivityResultCallback<R>
) = launch(Intent(context, T::class.java).putExtras(bundleOf(*values)), callback)
...
inline fun <reified T, R> PocketActivityResultLauncher<Intent, R>.launch(
    ...
    vararg values: Pair<String, Any?>,
    callback: ActivityResultCallback<R>
) = launch(Intent(context, T::class.java).putExtras(bundleOf(*values)), options, callback)